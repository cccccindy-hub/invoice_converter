<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Table Name (EN)" align="center" prop="tableEnName" />
      ...
      >
        <template #default="{ row }">
          <!-- Edit 按钮 -->
          <el-button
              v-if="row.tableIsgen !== 1 && row.tableStatus !== 1"
              size="default"
              type="text"
              @click="handleUpdate(row)"
              v-hasPermi="['datacenter:TableDefinition:edit']"
          >
            Edit
          </el-button>
          ...
          <el-button
              ...
              v-hasPermi="['datacenter:tableDefinition:editTemp']"
          >
            Edit
          </el-button>
          ...
          <el-button
              ...
              v-hasPermi="['columnConfigTemp:columnConfig:listTemp']"
          >
            Detail
          </el-button>
          ...
          <!-- Generate 按钮 -->
          <el-button
              v-if="row.tableStatus !== 1"
              size="default"
              type="text"
              @click="handleGenerate(row)"
              v-hasPermi="['datacenter:tableDefinition:generate']"
          >
            Run
          </el-button>
          ...
          <el-dropdown placement="top-start">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <!-- Syn 按钮 -->
                <el-dropdown-item v-if="row.tableSyn !== 1 && row.tableIsgen === 1">
                  <el-button
                      size="default"
                      type="text"
                      @click="handleSyn(row)"
                      v-hasPermi="['datacenter:tableDefinition:synTable']"
                  >
                    Syn
                  </el-button>
                </el-dropdown-item>
                ...
                <!-- Delete 按钮 -->
                <el-dropdown-item v-if="row.tableStatus !== 1">
                  <el-button
                      v-if="row.tableType!==99"
                      size="default"
                      type="text"
                      @click="handleDelete(row)"
                      v-hasPermi="['datacenter:tableDefinition:remove']"
                  >
                    Delete
                  </el-button>
                </el-dropdown-item>
                ...
                <!-- 回滚 按钮 -->
                <el-dropdown-item v-if="row.tableSyn !== 1 && row.tableStatus === 1">
                  <el-button
                      size="default"
                      type="text"
                      @click="handleRollBack(row)"
                      v-hasPermi="['datacenter:tableDefinition:rollback']"
                  >
                    RollBack
                  </el-button>
                </el-dropdown-item>
                ...
                <el-dropdown-item v-if="row.tableStatus === 1">
                  <el-button
                      ...
                      v-hasPermi="['datacenter:tableDefinition:stopTable']"
                  >
                    Stop
                  </el-button>
                </el-dropdown-item>
...
const getList = async () => {
  ...
  try {
    if(props.dcType === 3) {
      ...
      total.value = count;
    } else if(props.dcType === 9) {
      //财务报表accounts
      const {rows, total: count} = await financeService.financeConfigAccounts(queryParams);
      tableDefinitionList.value = rows;
      total.value = count;
    } else if(props.dcType === 10) {
      //财务报表reports
      const {rows, total: count} = await financeService.financeConfigReports(queryParams);
      tableDefinitionList.value = rows;
      total.value = count;
    } else if(props.dcType === 5) {
      queryParams.tableType = 5
