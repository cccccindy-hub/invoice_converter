...
<!--    />-->
    ...
    >
      ...
      <div style="margin-top: 15px;">
        <el-select v-model="selectedTableId"
...
const handleImport = async () => {
  ...
  formData.append("updateSupport", String(uploadParams.updateSupport));
  formData.append("leaderId", uploadParams.leaderId);
  formData.append("ignoreSheetName", String(uploadParams.ignoreSheetName));
  formData.append("validSheetPattern", uploadParams.validSheetPattern);  // 新增参数

  try {
    console.log(formData.values(),"开始导入");

    const response = await TableResultService.importTableData(
